...
#ACCESSKEY=fjFoUf64Fz39QKYwaceB28KTTgHzjuPTdQ7h_Jo3W6YvKE76dEFdPabuJfaao5v3
ACCESSKEY=Z_T0p2uGQNTSUNNRVmlvCYFVP4gicG2nSX4BqSk1G-GgTYYTBq8S8QQQB2stdA1x

ENGINE_JSON=/opt/PredictionIO/vendors/universal-recommender/engine.json
...
TRAIN_EXECUTOR_MEMORY=4G
TRAIN_DRIVER_MEMORY=4G
TRAIN_CORES=4
#
DEPLOY_EXECUTOR_MEMORY=4G
DEPLOY_DRIVER_MEMORY=4G
DEPLOY_CORES=4
...
TRAIN_COMMAND="${PIO_HOME}/bin/pio train -v $ENGINE_JSON
    -- $TRAIN_MASTER_PARAM
...
if [[ $TRAIN_RESULT -ne 0 ]]; then
  sendmail -s "Error in train: $NAME $TIMESTAMP" -a "From: $FROM_EMAIL" \
...
if [[ $RETURN_VAL -ne 0 ]]; then
  sendmail -s "Error in deploy: $NAME $TIMESTAMP" -a "From: $FROM_EMAIL" $TARGET_EMAIL < $LOG_FILE
else
  sendmail -s "Normal: $NAME $TIMESTAMP" -a "From: $FROM_EMAIL" $TARGET_EMAIL < $LOG_FILE
fi
